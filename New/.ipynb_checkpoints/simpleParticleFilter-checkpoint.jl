...
function recruits_pedigree(pedigrees,population)
    ...
    for p in pi_ls
        lL += log(population.pi[floor(Int,p)+1] + min_p)
    end 
    return lL
end 
...
log noraml o
"""
function estimateN(estimate,precision,population)
    N = log(population.N + 10^(-30))
...
"""
function estiamteH(nSample,nH,population)
    pH = population.H / (population.H + population.N)
    f = pdf(Distributions.Binomial(nSample,pH), nH)
    return log(f)
end 
...
struct data
    ...
    t_sampleH::AbstractVector{Int} 
    nSample::AbstractVector{Float64}
    nH::AbstractVector{Float64}
...
function likelihood(params, data; N_particles = 100, n = 4)
    ...
    for i in 1:N_particles
        push!(populaitons, pedigreeAproximation.init_simplePopulation(params.r,params.K, 0.0, params.s,n))
            
    end 
    pf = particleFilter(N_particles,logLikls,populaitons)
    
    t_data = unique(vcat(data.t_pedigree,data.t_recruitsN,data.t_sampleH))
    ...
    for t in 1:data.T_end
        ...
        for i in 1:N_particles
            ...
            if t in data.t_recruitsN
                pf.logLik[i] += estimateN(data.recruitsN[data.t_recruitsN .== t][1],data.precision,pf.particles[i])
            end 



            # hatchery migation
            if t in data.t_sampleH
                pf.logLik[i] += estiamteH(data.nSample[data.t_sampleH .== t][1],data.nH,pf.particles[i])
            end 
